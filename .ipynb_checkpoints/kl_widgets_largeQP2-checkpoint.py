...
SECOND_DATA_PATH1 = Path('/Prove/Yoshihisa/HEIF_ghost/KL_PLOT_WIDGETS/SECOND_sameQP_data/')
SECOND_DATA_PATH2 = Path('/Prove/Yoshihisa/HEIF_ghost/KL_PLOT_WIDGETS/SECOND_largeQP_data/')
TRIPLE_DATA_PATH = Path('/Prove/Yoshihisa/HEIF_ghost/KL_PLOT_WIDGETS/TRIPLE_largeQP_data/')
...
#         # 9, 10, 11の平均を求める
#         mean_9_10_11 = np.mean(unique_values[np.isin(unique_values, [9, 10, 11])])

#         # 25, 26, 27の平均を求める
#         mean_25_26_27 = np.mean(unique_values[np.isin(unique_values, [25, 26, 27])])

#         # 9, 10, 11, 25, 26, 27をunique_valuesから除外し、平均値をdesired_valuesに追加する
#         desired_values = np.delete(unique_values, np.where(np.isin(unique_values, [9, 10, 11, 25, 26, 27])))
#         desired_values = np.append(desired_values, [mean_9_10_11, mean_25_26_27])

#         # 対応する値のカウントを取得
#         corresponding_counts = counts[np.isin(unique_values, desired_values)]
...
#                 kl_divergence = calculate_kl_divergence((hist1[0][:min_len], hist1[1][:min_len]), 
#                                                         (hist2[0][:min_len], hist2[1][:min_len]))
#                 # print(f"file names1-{filename1}")
#                 # print(f"file names1-{filename2}")
#                 # print(f"KL Divergence-{types}: {kl_divergence}")
#                 kl_divergences.append(kl_divergence)

#     # print("The num of images: ",len(kl_divergences))
                
#     if kl_divergences:
        
#         average_kl_divergence = np.mean(kl_divergences)
#         # print(f"Average KL Divergence-{types}: {average_kl_divergence}")
        
#         # Sort kl_divergences in ascending order
#         kl_divergences_sorted = sorted(kl_divergences)
#         # print(kl_divergences_sorted)
...
#         # print(f"Q3: {q3}")
#         # print(f"Max: {max_val}")
#         # print([min_val, q1, median_val, q3, max_val])
#         a = [min_val, q1, median_val, q3, max_val]
#     return a
...
class Double_PlotVisualization1:
    def __init__(self):
    ...
    def extract_histograms(self, QP1, QP2, CTU, PRESET):
        hist_filenames = []
        hist_filenames.extend(list(second_histograms_by_index(SECOND_DATA_PATH1, QP1, QP2, CTU, PRESET)))
...
class Double_PlotVisualization2:
    def __init__(self):
    ...
    def extract_histograms(self, QP1, QP2, CTU, PRESET):
        hist_filenames = []
        hist_filenames.extend(list(second_histograms_by_index(SECOND_DATA_PATH2, QP1, QP2, CTU, PRESET)))
